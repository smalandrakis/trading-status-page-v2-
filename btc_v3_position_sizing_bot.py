"""
BTC V3 Position Sizing Trading Bot

Uses btc_model_package.predictor.BTCPredictor (V3 ensemble: 2h/4h/6h models)
with confidence-based position sizing: size = (confidence - 0.60) × 20, capped 1-5x.
...
- Thresholds: LONG >0.65, SHORT <0.25 (optimized for position sizing)
- Walk-forward validated: 45.5% WR, +$9.63 avg P&L per trade, +$7,047 total (2yr)
- Features: 22 (support/resistance, ATR, volume MAs, trend, RSI, BB, time encoding)
- Input: 250 bars of 5-min OHLCV from Binance

Position sizing:
- size = (confidence - 0.60) × 20, capped [1, 5]
- Higher confidence → more contracts per trade
- Avg position ~2.7x in backtest

IB clientId=405, port 4002 (paper)
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/btc_v3_position_sizing.log', mode='a'),
...
# Optimized thresholds for position sizing strategy
LONG_THRESHOLD = 0.65
SHORT_THRESHOLD = 0.25

# Confidence filter floor
MIN_CONFIDENCE = 0.65  # Matches LONG_THRESHOLD

# Position sizing parameters
SIZING_BASE = 0.60       # confidence offset
SIZING_FACTOR = 20       # multiplier
...
POSITION_FILE = "btc_v3_ps_positions.json"

# Trade database
DB_PATH = "v3_position_sizing_trades.db"
...
def calculate_position_size(confidence: float) -> int:
    ...
    Formula: size = (confidence - 0.60) × 20, capped [1, 5]
    Rounds to nearest integer for IB contract count.
    """
    raw_size = (confidence - SIZING_BASE) * SIZING_FACTOR
    capped = max(MIN_POSITION_SIZE, min(MAX_POSITION_SIZE, raw_size))
    return max(1, round(capped))
...
class PositionManager:
        ...
        if os.path.exists(self.filepath):
            try:
                ...
                for pos_id, pos in self.positions.items():
                    logger.info(f"  - {pos_id}: {pos.direction} {pos.size}x @ ${pos.entry_price:.2f} "
    ...
    def add_position(self, position: Position) -> None:
        ...
        logger.info(f"Added position [{position.model_id}]: {position.direction} "
                   f"{position.size}x @ ${position.entry_price:.2f}")
...
# MAIN BOT
# =============================================================================
class BTCV3PositionSizingBot:
    """BTC trading bot using V3 BTCPredictor ensemble + confidence-based position sizing + IB execution."""
    ...
    def __init__(self, paper_trading: bool = True):
        ...
        self.predictor.LONG_THRESHOLD = LONG_THRESHOLD
        self.predictor.SHORT_THRESHOLD = SHORT_THRESHOLD
        logger.info(f"BTCPredictor loaded (22-feature, LONG>{LONG_THRESHOLD} SHORT<{SHORT_THRESHOLD})")
        ...
        logger.info("=" * 60)
        logger.info("BTC V3 POSITION SIZING BOT INITIALIZED")
        logger.info("=" * 60)
        ...
        logger.info(f"Position sizing: ({SIZING_BASE}→conf) × {SIZING_FACTOR}, range [{MIN_POSITION_SIZE}-{MAX_POSITION_SIZE}]")
    ...
    def place_order(self, direction: str, size: int) -> Optional[Tuple[int, float]]:
        ...
        try:
            ...
            if trade.orderStatus.status == 'Filled':
                ...
                logger.info(f"Order filled: {action} {size}x @ ${fill_price:.2f}")
        ...
        try:
            ...
            if status == 'Filled':
                ...
                if position.direction == 'SHORT':
                    pnl = -pnl
                logger.info(f"Closed [{position.model_id}]: {position.size}x @ ${fill_price:.2f}, IB P&L: ${pnl:.2f}")
    ...
    def check_exits(self) -> None:
        ...
        for pos in positions:
            ...
            if should_exit:
                ...
                logger.info(f"EXIT [{pos.model_id}] {pos.direction} {pos.size}x: {reason}")
                ...
                if close_result == 'FLAT':
                    ...
                    # P&L accounting with position size
                    pnl_dollar = (pnl_pct / 100) * self.current_price * BTC_CONTRACT_VALUE * pos.size - (2.02 * pos.size)
    ...
    def _execute_entry(self, signal: str, confidence: float, details: dict, model_id: str) -> None:
        """Place order with confidence-based position sizing and create position."""
        ...
        # Calculate dynamic position size from confidence
        ...
        logger.info(f"ENTRY [{model_id}] {signal} {position_size}x: price=${entry_price:.2f}, "
                   f"conf={confidence:.3f}, TP=${tp_price:.2f}, SL=${sl_price:.2f}")
        logger.info(f"  Probabilities: {details.get('probabilities', {})}")
        logger.info(f"  Position sizing: conf={confidence:.3f} → ({confidence:.3f}-{SIZING_BASE})×{SIZING_FACTOR} = {position_size}x")

        # Place IB order with dynamic size
        ...
        logger.info(f"Position opened [{model_id}]: {signal} {position_size}x MBT @ ${entry_price:.2f}")
    ...
    def log_signal(self, signal: str, confidence: float, details: dict) -> None:
        ...
        try:
            ...
            log_path = os.path.join(SIGNAL_LOG_DIR, f"btc_v3_ps_signals_{today}.csv")

            file_exists = os.path.exists(log_path)
            with open(log_path, 'a') as f:
                if not file_exists:
                    f.write("timestamp,price,signal,confidence,avg_prob,prob_2h,prob_4h,prob_6h,position_size,positions\n")

                probas = details.get('probabilities', {})
                pos_size = calculate_position_size(confidence) if signal and signal != 'NEUTRAL' and confidence >= MIN_CONFIDENCE else 0
                row = [
    ...
    def run_iteration(self) -> None:
        ...
        if signal is not None:
            ...
            if signal != 'NEUTRAL':
                pos_size = calculate_position_size(confidence)
                action_str = f"{signal} conf={confidence:.3f} → {pos_size}x"
            else:
                action_str = 'NEUTRAL'
    ...
    def run(self) -> None:
        ...
        logger.info("Starting BTC V3 Position Sizing Bot...")
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description='BTC V3 Position Sizing Trading Bot')
    ...
    if args.test_signal:
        ...
        bot = BTCV3PositionSizingBot(paper_trading=True)
        ...
        if details:
            ...
            if signal and signal != 'NEUTRAL':
                ...
                logger.info(f"Position size: {pos_size}x (conf={confidence:.3f})")
                logger.info(f"TP: ${tp:,.2f} | SL: ${sl:,.2f}")
            else:
                logger.info("No trade signal — position size: 0")
        return

    bot = BTCV3PositionSizingBot(paper_trading=not args.live)
    bot.run()
