...
# Trailing stop params per direction
LONG_SL_PCT = 0.45
LONG_TS_ACT_PCT = 0.50
LONG_TS_TRAIL_PCT = 0.15

SHORT_SL_PCT = 0.20  # Mar 16: tightened from 0.30% — avg SL loss was $30 vs $15 avg win
SHORT_TS_ACT_PCT = 0.35  # Mar 16: widened from 0.25% — let winners run to improve R:R
SHORT_TS_TRAIL_PCT = 0.15  # Mar 13: widened from 0.05% — 0.05% locks in avg +$3 win vs $21 SL loss (1:7 R:R). 0.15% locks in ~$14 win matching SL
...
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'tick_trades.db')
PRICE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.btc_price')

logger = logging.getLogger('btc_tick_bot')
logger.setLevel(logging.INFO)

# File handler
fh = logging.FileHandler(os.path.join(LOG_DIR, 'btc_tick_bot.log'))
...
def log_open_position(db_path, pos_dict):
    conn = sqlite3.connect(db_path)
    cols = ', '.join(pos_dict.keys())
    placeholders = ', '.join(['?'] * len(pos_dict))
    conn.execute("INSERT INTO open_positions (%s) VALUES (%s)" % (cols, placeholders),
                 list(pos_dict.values()))
    conn.commit()
    conn.close()

def remove_open_position(db_path, model_id, entry_time):
    conn = sqlite3.connect(db_path)
...
def log_trade_event(db_path, event_dict):
    conn = sqlite3.connect(db_path)
    cols = ', '.join(event_dict.keys())
    placeholders = ', '.join(['?'] * len(event_dict))
    conn.execute("INSERT INTO trade_events (%s) VALUES (%s)" % (cols, placeholders),
...
class BTCTickBot:
    def __init__(self, paper_trading=True):
        ...
        logger.info("=" * 60)
        logger.info("BTC TICK ML BOT INITIALIZED")
        logger.info("=" * 60)
        ...
        def on_message(ws, message):
            try:
                ...
                if 'c' in data:
                    ...
                    # Write price to shared file for reverse bot
                    try:
                        with open(PRICE_FILE, 'w') as pf:
                            pf.write('%.2f\n' % price)
                    except:
                        pass
            except Exception as e:
    ...
    def check_entry(self, probs, feat=None):
        ...
        if n_long < MAX_LONG and n_pos < MAX_POSITIONS:
            ...
            if gap_ok and long_sl_ok:
                # Macro trend block: hard-block LONG if BTC down >2% in 24h
                if ret_24h is not None and ret_24h < MACRO_LONG_BLOCK_PCT:
                    logger.info("LONG blocked: 24h change %.2f%% < %.1f%% (macro downtrend)" % (ret_24h, MACRO_LONG_BLOCK_PCT))
                # Regular trend filter: block LONG if 1h return is too negative
                elif ret_1h is not None and ret_1h < TREND_FILTER_LONG:
                    logger.debug("LONG blocked: ret_1h=%.2f%% < %.2f%%" % (ret_1h, TREND_FILTER_LONG))
                else:
                    # Check thresholds (regular or trend-following mode)
                    long_threshold = LONG_THRESHOLD
                    if trend_follow_mode and ret_1h > TREND_FOLLOW_LONG_THRESHOLD:
                        long_threshold = TREND_FOLLOW_LONG_MODEL_THRESHOLD
                        logger.info("[%s] - using trend mode threshold %.0f%%" % (trend_follow_reason, long_threshold * 100))
                    
                    long_models = ['L1_full_wide']  # Mar 16: removed L3_mom_wide (25% WR, 8T, -$94)
                    for m in long_models:
                        if m in probs and probs[m] >= long_threshold:
                            self._enter_position('LONG', m, probs[m], probs)
                            break

        # --- SHORT: 2+ of S1, S3, S4 >= their thresholds ---
        ...
        if n_short < MAX_SHORT and n_pos < MAX_POSITIONS:
            ...
            if gap_ok and short_sl_ok:
                pass
            elif gap_ok and not short_sl_ok:
                gap_ok = False

            if gap_ok:
                # Macro trend block: hard-block SHORT if BTC up >2% in 24h
                if ret_24h is not None and ret_24h > MACRO_SHORT_BLOCK_PCT:
                    logger.info("SHORT blocked: 24h change +%.2f%% > +%.1f%% (macro uptrend)" % (ret_24h, MACRO_SHORT_BLOCK_PCT))
                    gap_ok = False
                # In strong uptrend, block all SHORT trades (trend-following)
                elif trend_follow_mode and ret_1h > TREND_FOLLOW_LONG_THRESHOLD:
                    logger.info("SHORT blocked: %s (uptrend - ride it)" % trend_follow_reason)
                    gap_ok = False
                # Regular trend filter: block SHORT if 1h return is too positive
                elif ret_1h is not None and ret_1h > TREND_FILTER_SHORT:
                    logger.debug("SHORT blocked: ret_1h=%.2f%% > %.2f%%" % (ret_1h, TREND_FILTER_SHORT))
                    gap_ok = False

            if gap_ok:
                # Check thresholds (regular or trend-following mode)
                short_threshold = 0.70  # Default threshold from model configs
                if trend_follow_mode and ret_1h < TREND_FOLLOW_SHORT_THRESHOLD:
                    short_threshold = TREND_FOLLOW_SHORT_MODEL_THRESHOLD
                    logger.info("[%s] - using trend mode threshold %.0f%%" % (trend_follow_reason, short_threshold * 100))
                ...
                for m in short_models:
                    if m in probs:
                        ...
                        # Use trend mode threshold if activated
                        if trend_follow_mode and ret_1h < TREND_FOLLOW_SHORT_THRESHOLD:
                            thresh = min(thresh, short_threshold)
                        if probs[m] >= thresh:
                            agrees += 1
                ...
                if agrees >= SHORT_AGREEMENT and trigger_model:
                    self._enter_position('SHORT', trigger_model, probs[trigger_model], probs, short_agreement=agrees)
    ...
    def _enter_position(self, direction, model_id, probability, all_probs, short_agreement=0):
        ...
        # Write to open_positions table for reverse bot to poll
        log_open_position(DB_PATH, {
            'model_id': model_id,
            'direction': direction,
        ...
        # Log ENTRY event for reverse bot signal-mirroring
        log_trade_event(DB_PATH, {
            'event_type': 'ENTRY',
            'model_id': model_id,
            'direction': direction,
            'price': price,
    ...
    def _close_position(self, pos, exit_price, reason):
        ...
        remove_open_position(DB_PATH, pos.model_id, pos.entry_time.isoformat())

        # Log EXIT event for reverse bot signal-mirroring
        log_trade_event(DB_PATH, {
            'event_type': 'EXIT',
    ...
    def log_signals(self, probs):
        ...
        if self.signal_csv_date != today:
            ...
            self.signal_csv_path = os.path.join(SIGNAL_LOG_DIR, 'btc_tick_signals_%s.csv' % today)
    ...
    def run(self):
        ...
        # Clear stale open_positions and trade_events from previous run
        try:
            conn = sqlite3.connect(DB_PATH)
            ...
            conn.commit()
            conn.close()
            logger.info("Cleared stale open_positions and trade_events tables")
        except Exception as e:
            logger.debug("startup cleanup: %s" % e)
