"""
BTC Trading Bot V2 - Bot-Managed TP/SL (No Bracket Orders)
...
# Setup logging
LOG_FILE = os.path.join(BOT_DIR, 'logs', 'btc_ib_bot_v2.log')
TRADE_LOG_FILE = os.path.join(BOT_DIR, 'logs', 'btc_trades_v2.jsonl')
...
IB_HOST = '127.0.0.1'
IB_PORT = 4002
IB_CLIENT_ID = 12  # NEW CLIENT ID for V2 bot
...
# =============================================================================
logger.info("=" * 80)
logger.info("BTC IB GATEWAY BOT V2 - BOT-MANAGED TP/SL")
logger.info("=" * 80)
...
try:
    while True:
        try:
            ...
            # ADOPT ORPHAN POSITIONS - Close immediately
            elif ADOPT_ORPHANS and ib_pos and ib_pos['quantity'] != 0:
                # Found orphan position in IB - close it immediately
                quantity = abs(ib_pos['quantity'])
                direction = 'LONG' if ib_pos['quantity'] > 0 else 'SHORT'

                logger.info(f"\n{'='*80}")
                logger.info(f"⚠️  ORPHAN POSITION DETECTED - CLOSING IMMEDIATELY")
                logger.info(f"Direction: {direction}")
                logger.info(f"Size: {quantity} contracts")
                logger.info(f"Current IB Price: ${ib_price:,.2f}")
                logger.info(f"{'='*80}\n")

                # Log orphan detection
                log_trade('ORPHAN_DETECTED', {
                    'direction': direction,
                    'position_size': quantity,
                ...
                logger.info(f"✓ Orphan position closed at ${ib_price:,.2f}")

                # Log closure (without P&L since we don't know real entry)
                log_trade('ORPHAN_CLOSED', {
                    'direction': direction,
                    'position_size': quantity,
                    'close_price': ib_price,
