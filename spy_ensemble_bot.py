"""
SPY/MES Ensemble Trading Bot - Using MNQ V2 Models (Dec 20, 2025).

Uses MNQ models (trained on QQQ) to trade S&P500 (SPY/MES).
Models transfer well due to high correlation between QQQ and SPY.

Models Used (from models_mnq_v2/):
- 4h_0.5% LONG: 97.6% WR on SPY backtest
- 2h_0.5% SHORT: 100% WR on SPY backtest  
- 4h_0.5% SHORT: 90.9% WR on SPY backtest
- 2h_0.5% LONG: EXCLUDED (31% WR - doesn't transfer)

Optimized Parameters:
- SL: 0.75%
- Trailing: DISABLED (better results without)
- Target: 0.5%
- Timeout: 2x horizon
- Probability threshold: 65%

Backtest Results (Oct 10 - Dec 17, 2025):
- 39 trades, 100% win rate
- +22.55% P&L in 68 days
- 0 stop losses hit

Features:
- Position persistence (survives restarts)
- Orphan position detection
- Real-time SPY data from Finnhub
- MES futures trading via IB Gateway
- Contract auto-rollover for MES futures
"""
...
# Finnhub API for real-time SPY quotes (free tier: 60 calls/min)
FINNHUB_API_KEY = os.environ.get('FINNHUB_API_KEY', '')  # Set in environment or here
MES_SPY_RATIO = 10.0  # MES/SPY price ratio (ES ~6000, SPY ~600)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/spy_bot.log', mode='a'),  # Append mode
...
# SPY Ensemble configuration - Using MNQ V2 models that transfer well to SPY
# NOTE: 2h LONG excluded (31% WR on SPY - doesn't transfer)
ENSEMBLE_MODELS_LONG = [
    # 2h LONG excluded - doesn't transfer to SPY
    {'horizon': '4h', 'threshold': 0.5, 'horizon_bars': 48, 'priority': 1, 'direction': 'LONG'},
]

# SPY SHORT models - both transfer excellently
...
# Combined models (3 total: 1 LONG, 2 SHORT)
...
# Trailing stop DISABLED for SPY - backtest shows better results without
# With trailing: 47 trades, 89.4% WR, +19.39% P&L
# Without trailing: 39 trades, 100% WR, +22.55% P&L
TRAILING_STOP_ENABLED = False
TRAILING_STOP_PCT = 0.15  # Not used when disabled
TRAILING_STOP_ACTIVATION_PCT = 0.15  # Not used when disabled

# Stop loss percentage (updated Jan 7, 2026)
# Extended backtest (3 months) showed SL=0.5%, TP=1.0% gives 79% WR, +$344/month
STOP_LOSS_PCT = 0.50
TAKE_PROFIT_PCT = 1.0  # Added explicit TP for ML models

# Position limits per direction (1 LONG model, 2 SHORT models)
...
# Indicator-based LONG strategy configuration (Jan 6, 2026)
# Strategy: RSI < 40 AND BB %B < 0.4 (double confirmation mean reversion)
# Uses QQQ parquet features (same as MNQ) for signal generation
# Backtest (30 days): 97 trades, 51.5% WR, $66/week, 3.5 trades/day
INDICATOR_LONG_ENABLED = True
INDICATOR_LONG_SL_PCT = 0.50  # 0.5% stop loss
INDICATOR_LONG_TP_PCT = 1.00  # 1.0% take profit (2:1 R:R)
INDICATOR_LONG_TIMEOUT_BARS = 48  # 4 hour timeout
INDICATOR_LONG_RSI_THRESHOLD = 40  # RSI must be below this
INDICATOR_LONG_BB_THRESHOLD = 0.4  # BB %B must be below this
MAX_INDICATOR_LONG_POSITIONS = 1  # Separate limit for indicator LONG positions

# ML V3 Model configuration (Jan 6, 2026)
# Model: 1.0% target, 48 bars horizon, trained on 30-day QQQ data
# Backtest: 53.3% WR, $34/week, ~0.5 trades/day
ML_V3_ENABLED = True
ML_V3_MODEL_PATH = "models_mnq_spy_v3/model_long_1.0pct_48bars.joblib"
ML_V3_FEATURE_PATH = "models_mnq_spy_v3/feature_columns.json"
ML_V3_PROB_THRESHOLD = 0.55  # 55% probability threshold
ML_V3_SL_PCT = 1.00  # 1.0% stop loss
ML_V3_TP_PCT = 2.00  # 2.0% take profit (2:1 R:R)
ML_V3_TIMEOUT_BARS = 96  # 8 hour timeout (2x horizon)
MAX_ML_V3_POSITIONS = 1  # Separate limit for ML V3 positions

# Historical end values for cumulative features - will be computed from SPY data
# These are placeholders - actual values computed on first data fetch
CUMULATIVE_HIST_END_SPY = {
    "volume_adi": 0,  # Will be computed from SPY parquet
    "volume_obv": 0,
    "volume_nvi": 0,
    "volume_vpt": 0,
    "others_cr": 0,
...
class SignalLogger:
    ...
    def __init__(self, log_dir: str = SIGNAL_LOG_DIR, prefix: str = "spy"):
    ...
    def _init_csv(self) -> None:
        ...
        if today != self.current_date:
            ...
            if not os.path.exists(self.csv_path):
                header = [
                    'timestamp', 'spy_price', 'mes_price', 'price_source',
                    # LONG models (only 4h for SPY)
                    'prob_4h_0.5pct',
                    'signal_4h_0.5pct',
                    # SHORT models (2h and 4h)
    ...
    def log_signals(self, timestamp: datetime, spy_price: float, mes_price: float,
        ...
        # Model order must match header - LONG models (only 4h for SPY)
        long_models = ['4h_0.5pct']
        # SHORT models
...
class PositionManager:
    ...
    def __init__(self, filepath: str = "spy_positions.json"):
...
def get_front_month_expiry() -> str:
    ...
    MES expiries are on 3rd Friday of Mar, Jun, Sep, Dec (H, M, U, Z months).
    """
    now = datetime.now()
    year = now.year
    month = now.month
...
class EnsembleTradingBot:
    ...
    def __init__(self,
                 probability_threshold: float = 0.50,  # Lowered from 0.65 on Jan 16, 2026
                 position_size: int = 1,  # Number of contracts (MES) or shares (SPY)
                 ...
                 max_positions: int = 3,  # max concurrent positions (1 LONG + 2 SHORT)
                 instrument: str = 'MES'):  # 'MES' for futures, 'SPY' for stock
        ...
        # Load ML V3 model (Jan 6, 2026)
        self.ml_v3_model = None
        self.ml_v3_feature_cols = None
        if ML_V3_ENABLED:
            self._load_ml_v3_model()

        # Contract setup based on instrument
        if instrument == 'MES':
            expiry = get_front_month_expiry()
            self.contract = Future('MES', expiry, 'CME')
            self.point_value = 5  # $5 per point for MES
            self.symbol = 'MES'
            # For data, we use SPY (model trained on QQQ, but SPY correlates well)
            self.data_contract = Stock("SPY", "SMART", "USD")
        else:
        ...
        # Real-time price - separate tickers for trading (MES) and data (SPY)
        self.current_price = None
        self.ticker = None  # For trading contract (MES)
        self.data_ticker = None  # For data contract (SPY) - used for synthetic bar
        ...
        # Signal logging
        self.signal_logger = SignalLogger(prefix="spy")
        ...
        logger.info("="*60)
        logger.info("SPY/MES ENSEMBLE TRADING BOT INITIALIZED")
        logger.info("="*60)
        logger.info(f"Instrument: {instrument}")
        if instrument == 'MES':
            logger.info(f"  Contract: MES {get_front_month_expiry()} (Micro E-mini S&P500 Futures)")
            logger.info(f"  Point value: ${self.point_value}/point")
        logger.info(f"Models: {len(self.models)} (using MNQ v2 models)")
        for m in ENSEMBLE_MODELS:
            logger.info(f"  - {m['horizon']}_{m['threshold']}% {m['direction']} (priority {m['priority']})")
        logger.info(f"Probability threshold: {probability_threshold}")
        logger.info(f"Position size: {position_size} {'contracts' if instrument == 'MES' else 'shares'}")
    ...
    def _load_feature_columns(self) -> List[str]:
        ...
        # Last fallback: return empty (will use model's expected features)
        logger.warning("No feature columns file found - using default from model")
        return []
    
    def _load_ml_v3_model(self):
        """Load ML V3 model for 1.0% target LONG signals (Jan 6, 2026)."""
        try:
            if os.path.exists(ML_V3_MODEL_PATH):
                self.ml_v3_model = joblib.load(ML_V3_MODEL_PATH)
                logger.info(f"Loaded ML V3 model from {ML_V3_MODEL_PATH}")
                
                # Load feature columns
                if os.path.exists(ML_V3_FEATURE_PATH):
                    with open(ML_V3_FEATURE_PATH, 'r') as f:
                        self.ml_v3_feature_cols = json.load(f)
                    logger.info(f"Loaded {len(self.ml_v3_feature_cols)} ML V3 feature columns")
                elif hasattr(self.ml_v3_model, 'feature_names_in_'):
                    self.ml_v3_feature_cols = list(self.ml_v3_model.feature_names_in_)
                    logger.info(f"Using {len(self.ml_v3_feature_cols)} features from model")
            else:
                logger.warning(f"ML V3 model not found: {ML_V3_MODEL_PATH}")
                self.ml_v3_model = None
        except Exception as e:
            logger.error(f"Error loading ML V3 model: {e}")
            self.ml_v3_model = None
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.ib.connect(config.IB_HOST, port, clientId=config.IB_CLIENT_ID + 2)  # Client ID 3 for SPY bot
            ...
            if self.data_contract != self.contract:
                ...
                logger.info(f"Subscribed to market data for SPY (for features)")
            else:
    ...
    def get_realtime_data_price(self) -> Optional[float]:
        """Get current SPY price from real-time feed (for features/synthetic bar)."""
    ...
    def get_finnhub_spy_price(self) -> Optional[float]:
        """Get real-time SPY price from Finnhub API (free, 60 calls/min).
        
        Returns SPY price or None if unavailable.
        """
    ...
    def get_yahoo_es_price(self) -> Optional[float]:
        """Get real-time ES price from Yahoo Finance.
        
        ES futures trade nearly 24/7, so this works overnight when SPY is closed.
        Returns ES price or None if unavailable.
        """
        try:
            import yfinance as yf
            es = yf.Ticker("ES=F")
            # Get latest 5-min bar
            hist = es.history(period="1d", interval="5m")
            if len(hist) > 0:
                price = float(hist['Close'].iloc[-1])
                if price > 0:
                    return price
        except Exception as e:
            logger.debug(f"Yahoo ES price error: {e}")
        return None
    ...
    def get_live_mes_price(self) -> Optional[float]:
        """Get best available MES price estimate.
        
        Priority during US market hours (9:30-16:00 ET):
        1. Finnhub real-time SPY * ratio (fast, reliable)
        2. IB historical SPY last bar * ratio (0-5 min old)
        3. IB delayed MES ticker (15 min old)
        
        Priority outside market hours (overnight/pre-market):
        1. Yahoo Finance ES=F (actual futures price, trades 24/7)
        2. IB delayed MES ticker
        """
        # Check if we're in US market hours
        if self.is_us_market_hours():
            # During market hours: use Finnhub SPY (faster, more reliable)
            spy_price = self.get_finnhub_spy_price()
            if spy_price:
                mes_price = spy_price * MES_SPY_RATIO
                logger.debug(f"Finnhub SPY: ${spy_price:.2f} -> MES: ${mes_price:.2f}")
                return mes_price
            
            # Fallback to IB historical (0-5 min old)
            if not self.data_cache.empty:
                spy_price = self.data_cache['close'].iloc[-1]
                mes_price = spy_price * MES_SPY_RATIO
                logger.debug(f"IB historical SPY: ${spy_price:.2f} -> MES: ${mes_price:.2f}")
                return mes_price
        else:
            # Outside market hours: use Yahoo ES=F (actual futures price)
            mes_price = self.get_yahoo_es_price()
            if mes_price:
                logger.debug(f"Yahoo ES=F (overnight): ${mes_price:.2f}")
                return mes_price
    ...
    def load_parquet_features(self) -> bool:
        ...
        Uses QQQ parquet since SPY bot uses MNQ models (trained on QQQ).
        This provides correct cumulative feature values computed from full history.
        Returns True if successful, False otherwise.
        """
        try:
            # Use QQQ parquet - SPY bot uses MNQ models trained on QQQ
    ...
    def refresh_parquet_from_yahoo(self) -> bool:
        """Refresh parquet with latest NQ Futures data from Yahoo Finance.
        
        Uses NQ=F (E-mini Nasdaq Futures) which trades 24/7, converted to QQQ scale.
        This ensures consistent data for both market hours and overnight trading.
        ...
        try:
            ...
            logger.info("=" * 60)
            logger.info("PARQUET REFRESH: Fetching NQ Futures from Yahoo Finance")
            logger.info("=" * 60)
            
            # Load existing parquet OHLCV (QQQ scale - SPY uses MNQ models trained on QQQ)
            parquet_path = 'data/QQQ_features.parquet'
            existing = pd.read_parquet(parquet_path)
            last_time = existing.index[-1]
            # Make timezone-naive for comparison with Yahoo data
            ...
            # Fetch NQ Futures data (trades 24/7, unlike QQQ ETF)
            nq = yf.Ticker('NQ=F')
            new_data = nq.history(period='5d', interval='5m')
            
            if len(new_data) == 0:
                logger.warning("PARQUET REFRESH: No NQ Futures data, trying QQQ fallback")
                qqq = yf.Ticker('QQQ')
                new_data = qqq.history(period='1mo', interval='5m')
                if len(new_data) == 0:
                ...
                # Convert NQ Futures prices to QQQ scale (NQ / 41.2)
                # NQ is ~25700, QQQ is ~624, ratio is ~41.2
                NQ_QQQ_RATIO = 41.2
                for col in ['Open', 'High', 'Low', 'Close']:
                    new_data[col] = new_data[col] / NQ_QQQ_RATIO
                logger.info(f"Converted NQ Futures to QQQ scale (÷{NQ_QQQ_RATIO})")
            
            # Clean up Yahoo data - convert to timezone-naive to match parquet
            ...
            logger.info(f"Yahoo NQ data: {len(new_data)} bars, latest at {yahoo_latest}")
            ...
            for feat in cumulative_feats:
                if feat in df.columns:
                    CUMULATIVE_HIST_END_SPY[feat] = df[feat].iloc[-1]
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if self.cumulative_offsets is None:
                ...
                for feat, hist_end in CUMULATIVE_HIST_END_SPY.items():
                    if feat in df.columns:
    ...
    def get_features_with_synthetic_bar(self) -> pd.DataFrame:
        ...
        if self.use_parquet_features and self.parquet_features is not None:
            ...
            parquet_last = self.parquet_last_time
            if hasattr(parquet_last, 'tzinfo') and parquet_last.tzinfo is not None:
                parquet_last = parquet_last.replace(tzinfo=None)
            if hasattr(synthetic_time, 'tzinfo') and synthetic_time.tzinfo is not None:
                synthetic_time_naive = synthetic_time.replace(tzinfo=None)
            else:
                synthetic_time_naive = synthetic_time
            
            # If synthetic bar is newer than parquet, append it
            if synthetic_time_naive > parquet_last:
                ...
                if hasattr(synthetic_time, 'tzinfo') and synthetic_time.tzinfo is not None:
                    synthetic_time = synthetic_time.replace(tzinfo=None)
    ...
    def get_indicator_long_signals(self, df: pd.DataFrame) -> List[Dict]:
        """Get LONG signals based on RSI + BB double confirmation.
        
        Strategy (Jan 6, 2026): RSI < 40 AND BB %B < 0.4
        Uses QQQ parquet features (same data source as MNQ bot).
        - Backtest (30 days): 97 trades, 51.5% WR, $66/week, 3.5 trades/day
        - SL: 0.50%, TP: 1.00% (2:1 R:R ratio)
        """
        signals = []
        ...
        try:
            ...
            # Get indicator values from QQQ features
            bb_pct_b = last_row.get('volatility_bbp', 0.5)
            rsi = last_row.get('momentum_rsi', 50)
            
            # Check conditions: RSI < 40 AND BB %B < 0.4 (double confirmation)
            rsi_condition = rsi < INDICATOR_LONG_RSI_THRESHOLD
            bb_condition = bb_pct_b < INDICATOR_LONG_BB_THRESHOLD
            ...
            if rsi_condition and bb_condition:
                ...
                logger.info(f"INDICATOR LONG signal: RSI={rsi:.1f} < {INDICATOR_LONG_RSI_THRESHOLD}, BB %B={bb_pct_b:.4f} < {INDICATOR_LONG_BB_THRESHOLD}")
    ...
    def get_ml_v3_signals(self, df: pd.DataFrame) -> List[Dict]:
        """Get LONG signals from ML V3 model (1.0% target, 48 bars horizon).
        
        Model trained Jan 6, 2026 on 30-day QQQ data.
        Backtest: 53.3% WR, $34/week, ~0.5 trades/day
        """
        signals = []
        
        if not ML_V3_ENABLED or self.ml_v3_model is None:
            return signals
        ...
        try:
            ...
            # Prepare features for ML V3 model
            # The model needs specific features - add any missing ones
            X = pd.DataFrame(index=last_row.index)
            
            for col in self.ml_v3_feature_cols:
                if col in last_row.columns:
                    X[col] = last_row[col].values
                else:
                    # Try to compute missing features
                    if col == 'macd_hist':
                        X[col] = (last_row.get('trend_macd', 0) - last_row.get('trend_macd_signal', 0)).values
                    elif col == 'macd_hist_change':
                        X[col] = 0  # Would need previous bar
                    elif col == 'rsi_change':
                        X[col] = 0
                    elif col == 'stoch_change':
                        X[col] = 0
                    elif col.startswith('roc_'):
                        X[col] = 0
                    elif col.startswith('price_vs_'):
                        X[col] = 0
                    elif col == 'atr_pct':
                        atr = last_row.get('volatility_atr', 0)
                        close = last_row.get('Close', 1)
                        X[col] = (atr / close * 100).values if close.values[0] > 0 else 0
                    elif col == 'bb_width_pct':
                        X[col] = last_row.get('volatility_bbw', 0).values
                    elif col == 'range_pct':
                        high = last_row.get('High', 0)
                        low = last_row.get('Low', 0)
                        close = last_row.get('Close', 1)
                        X[col] = ((high - low) / close * 100).values if close.values[0] > 0 else 0
                    elif col == 'bb_position':
                        X[col] = last_row.get('volatility_bbp', 0.5).values
                    elif col == 'bb_squeeze':
                        X[col] = 0
                    elif col == 'volume_ratio':
                        X[col] = 1.0
                    elif col in ['rsi_oversold', 'rsi_overbought', 'stoch_oversold', 'stoch_overbought']:
                        X[col] = 0
                    elif col in ['is_morning', 'is_afternoon']:
                        hour = datetime.now().hour
                        X[col] = 1 if (col == 'is_morning' and 9 <= hour < 12) or (col == 'is_afternoon' and 12 <= hour < 16) else 0
                    else:
                        X[col] = 0
            ...
            # Get probability
            prob = self.ml_v3_model.predict_proba(X)[0, 1]
            
            if prob >= ML_V3_PROB_THRESHOLD:
                signals.append({
                    'model_id': 'ml_v3_long',
                    'horizon': '48bars',
                    'threshold': ML_V3_TP_PCT,
                    'probability': prob,
                    'priority': 1,  # Lower priority than indicator
                    'target_bars': ML_V3_TIMEOUT_BARS,
                    'direction': 'LONG',
                    'signal_type': 'ml_v3',
                    'sl_pct': ML_V3_SL_PCT,
                    'tp_pct': ML_V3_TP_PCT
                })
                logger.info(f"ML V3 LONG signal: prob={prob:.1%} >= {ML_V3_PROB_THRESHOLD:.0%}")
        
        except Exception as e:
            logger.error(f"Error getting ML V3 signals: {e}")
    ...
    def enter_indicator_position(self, signal: Dict, current_price: float) -> bool:
        ...
        if order_id:
            ...
            unit = "contracts" if self.instrument == 'MES' else "shares"
            risk_per_contract = abs(current_price - stop_price) * self.point_value
            total_risk = risk_per_contract * self.position_size
            ...
            logger.info(f"Risk: ${total_risk:.2f}")
            logger.info(f"Max hold: {max_hold_bars * 5} mins ({max_hold_bars} bars)")
            logger.info(f"Total positions: {self.position_manager.count_positions()}")
            logger.info("="*50)
            return True
    ...
    def enter_ml_v3_position(self, signal: Dict, current_price: float) -> bool:
        """Enter a position for ML V3 model signal (1.0% target LONG)."""
        model_id = signal['model_id']
        ...
        # Check ML V3 position limit
        ml_v3_positions = sum(1 for pos in self.position_manager.positions.values() 
                              if pos.model_id.startswith('ml_v3_'))
        if ml_v3_positions >= MAX_ML_V3_POSITIONS:
            return False
        ...
        block_entries, block_reason = market_hours.should_block_new_entries(min_holding_time_minutes=60)
        if block_entries:
            logger.debug(f"Blocking ML V3 entry: {block_reason}")
            return False
        
        # Use ML V3-specific SL/TP
        sl_pct = signal['sl_pct']
        tp_pct = signal['tp_pct']
        max_hold_bars = signal['target_bars']
        ...
        if order_id:
            ...
            total_risk = risk_per_share * self.position_size * self.point_value
            
            unit = "contracts" if self.instrument == 'MES' else "shares"
            
            logger.info("="*50)
            logger.info(f"ENTERED LONG [{model_id}]: {self.position_size} {unit} @ {current_price:.2f} (ML V3)")
    ...
    def _get_current_price_for_logging(self) -> float:
        """Get current MES price for trade logging."""
        try:
            # Try Finnhub first
            spy_price = self.get_finnhub_spy_price()
            if spy_price:
                return spy_price * MES_SPY_RATIO
        ...
        try:
            ...
            for model_id, order_time in list(self.pending_close_orders.items()):
                ...
                if mnq_position == 0:
                    ...
                    # Log trade to database before removing position
                    try:
                        current_price = self._get_current_price_for_logging()
                        ...
                        pnl = price_diff * pos.size * self.point_value
                        ...
                        logger.warning(f"Failed to log pending close trade: {e}")
    ...
    def run_iteration(self, full_check: bool = False) -> None:
        ...
        if new_bar or full_check:
            ...
            # Check if parquet needs hourly refresh from Yahoo Finance
            self.maybe_refresh_parquet()
        ...
        if is_market_hours:
            spy_price = self.get_finnhub_spy_price()
            if spy_price:
                price_source = "finnhub_spy"
            else:
                spy_price = self.data_cache['close'].iloc[-1]
                price_source = "ib_cache"
        else:
            # Overnight: get ES price and convert to SPY scale for features
            mes_price_yahoo = self.get_yahoo_es_price()
            if mes_price_yahoo:
                spy_price = mes_price_yahoo / MES_SPY_RATIO
                price_source = "yahoo_es"
                logger.debug(f"Overnight: Yahoo ES ${mes_price_yahoo:.2f} -> SPY ${spy_price:.2f}")
            else:
                spy_price = self.data_cache['close'].iloc[-1]
                price_source = "ib_cache_stale"
        
        # Update synthetic bar with real-time SPY price (kept for price tracking only)
        self.update_synthetic_bar(spy_price)
        ...
        if new_bar or self.last_validation_time is None:
            self.validate_data_for_signals(df, spy_price)  # Logs warnings/errors but continues
        
        # Apply feature normalization if using normalized models
        if USE_NORMALIZED_MODELS:
            df = normalize_features(df)
        
        # Get live MES price for trading (Yahoo ES=F preferred, fallback to SPY ratio)
        trading_price = self.get_live_mes_price()
        if trading_price is None:
            trading_price = spy_price * MES_SPY_RATIO
        ...
        if self.voting_mode == 'parallel':
            ...
            # Log EVERY signal for SPY bot (for uptime monitoring)
            logger.info(f"MES: ${trading_price:.2f} | SPY: ${spy_price:.2f} | Pos: {n_pos}/{self.max_positions} | {prob_str}")
            ...
            # Second pass: enter positions for signals above threshold
            for model_name, model_data in self.models.items():
            ...
            # Uses QQQ parquet features (same as MNQ bot) for signal generation
            ...
            # Check for ML V3 LONG signals (1.0% target model - Jan 6, 2026)
            ml_v3_signals = self.get_ml_v3_signals(df)
            for ml_v3_signal in ml_v3_signals:
                if self.enter_ml_v3_position(ml_v3_signal, trading_price):
                    self.total_signals += 1
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SPY/MES Ensemble Trading Bot (using MNQ models)')
    parser.add_argument('--instrument', type=str, default='MES',
                        choices=['MES', 'SPY'],
                        help='Trading instrument (default: MES futures)')
    parser.add_argument('--prob-threshold', type=float, default=0.65,
                        help='Min probability to enter (default: 0.65)')
    parser.add_argument('--position-size', type=int, default=1,
                        help='Contracts (MES) or shares (SPY) per trade (default: 1)')
    ...
    parser.add_argument('--max-positions', type=int, default=3,
                        help='Max concurrent positions in parallel mode (default: 3)')
